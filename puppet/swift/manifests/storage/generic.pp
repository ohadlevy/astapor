...
define swift::storage::generic(
  ...
  service { "swift-${name}-auditor":
    name      => inline_template("<%= scope.lookupvar('::swift::params::${name}_auditor_service_name') %>"),
    ensure    => running,
    enable    => true,
    hasstatus => true,
    provider  => $service_provider,
  ...
  if "${name}" != "account"{
    service { "swift-${name}-updater":
      name      => inline_template("<%= scope.lookupvar('::swift::params::${name}_updater_service_name') %>"),
      ensure    => running,
      enable    => true,
      hasstatus => true,
      provider  => $service_provider,
  ...
  if "${name}" == "account"{
    service { "swift-${name}-reaper":
      name      => inline_template("<%= scope.lookupvar('::swift::params::${name}_reaper_service_name') %>"),
      ensure    => running,
      enable    => true,
      hasstatus => true,
      provider  => $service_provider,
